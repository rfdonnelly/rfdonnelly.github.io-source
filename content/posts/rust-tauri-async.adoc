...
More specifically we want bidirectional communication between the Tauri webview and an async Rust process.
...
The async process model will take input via a mpsc (Multi-Producer, Single-consumer) channel and give output via another mpsc channel.
...
The Tauri docs do a good job covering both.
...
* JavsScript initiates all communication
