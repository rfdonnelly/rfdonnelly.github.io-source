...
More specifically, perform bidirectional communication between the Tauri webview and an async Rust process.
...
We'll keep it really basic to make this more generally applicable to various applications.
...
The async process model will take input via a https://docs.rs/tokio/latest/tokio/sync/mpsc/index.html[`tokio::mpsc`] (Multi-Producer, Single-consumer) channel and give output via another `tokio::mpsc` channel.
The model is a simple loop that takes strings from the input channel and returns them on the output channel.
...
In this tutorial, the goal is to allow either side to initiate communication.
...
I rewrote the autogenerated `HelloWorld` Vue component to provide an interface for sending messages to Rust and reporting messages in both directions.
